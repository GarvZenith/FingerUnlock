...
- ✅ **Packaging** — one-click **`setup.bat`** / **`install.ps1`** (registers CP, installs the boot-SYSTEM service, generates a token, opens firewall + Tailscale) + **self-contained release package** (`FingerUnlock-v1.0.0-Windows.zip`). Phone = prebuilt APK (`FingerUnlock-v1.0.0-Android.apk`). See [GitHub Releases](https://github.com/GarvZenith/FingerUnlock/releases) & `docs/User_Installation_Guide.md`. ✔ tested
